...
<?php while ($query->have_posts()) : $query->the_post(); ?>
    <?php if ($index % $contentSource->content_settings->rows == 0): ?>
        ...
        <?php endif; ?>
            ...
            <div class="feature-image" style="<?php echo 'height:'.esc_attr($contentSource->content_settings->item_width).'px;';?>">
                            ...
                            <div class="product-action">
                                <?php if ($contentSource->content_settings->add_to_wishlist && defined('YITH_WCWL_TABLE')): ?>
                                    <?php if ($bt_utility->is_product_in_wishlist(get_the_ID())): ?>
                                        <span class="view-wishlist" title="<?php echo __('View Wishlist', 'ap-background'); ?>" data-link="<?php echo get_permalink(esc_attr(get_option('yith_wcwl_wishlist_page_id'))); ?>"><i class="fa fa-heart"></i></span>
                                    <?php else: ?>
                                        <span class="wishlist" title="<?php echo __('Add wishlist', 'ap-background'); ?>" data-pid="<?php echo get_the_ID(); ?>"><i class="fa fa-heart"></i></span>
                                        <span class="view-wishlist hidden" title="<?php echo __(' View Wishlist', 'ap-background'); ?>" data-link="<?php echo get_permalink(esc_attr(get_option('yith_wcwl_wishlist_page_id'))); ?>"><i class="fa fa-heart"></i></span>
                                    <?php endif; ?>
                                <?php endif ?>
                                <?php
                                if ($contentSource->content_settings->add_to_cart):
                                    ...
                                    if ($product_type[0]->name == 'simple'):
                                        ?>
                                        <span class="addcart last" title="<?php echo __('Add cart', 'ap-background'); ?>" data-pid="<?php echo get_the_ID(); ?>"><i class="fa fa-shopping-cart"></i></span>
                                    <?php else: ?>
                                        <span class="viewproduct last" title="<?php echo ($product_type[0]->name == 'variable') ? __('Select option', 'ap-background') : __('View product', 'ap-background'); ?>" data-link="<?php echo the_permalink(); ?>"><?php echo ($product_type[0]->name == 'variable') ? '<i class="fa fa-bars"></i>' : '<i class="fa fa-info"></i>'; ?></span>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </div>
                            <div class="ajax-content">
                                <span class="loadding"><img src="<?php echo SFAPB_PLUGIN_URL.'assets/images/loading.gif'; ?>"  alt="<?php echo __('Loading...', 'ap-background');?>"/></span>
                        ...
                        <div class="post-info">
                            ...
                            if (!empty($post_categories)) {
                                for ($i = 0; $i < count($post_categories); $i++) {
                                    ...
                                    echo '<span class="link"><a href="' . get_term_link($pcat) . '">' . esc_attr($pcat->name) . '</a></span> ';
                                }
                            }
                            ?>
                        </div>
