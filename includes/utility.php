...
class btParallaxBackgroundUtility {
    ...
    function getMessage($message, $type = 'success') {
        ...
        $html[] = '<div class="bt-message ' . esc_attr($class) . '">';
        $html[] = '<div class="message-text">' . wp_kses_post($message) . '</div>';
        $html[] = '</div>';
        return implode($html);
    }
    ...
    function selectFieldRender($id, $name, $value, $data, $text = '', $class = '', $multi = true) {
        ...
        if ($multi) {
            ...
            $html[] = '<select id="' . esc_attr($id) . '" class="' . esc_attr($class) . '" name="' . esc_attr($name) . '[]" ' . esc_attr($multiselect) . '>';
            if ($text) {
                $html[] = '<option value="">' . esc_attr($text) . '</option>';
            }
        } else {
            $html[] = '<select class="' . esc_attr($class) . '" name="' . esc_attr($name) . '" id="' . esc_attr($id) . '">';
            if ($text) {
                $html[] = '<option value="">' . esc_attr($text) . '</option>';
            }
        }
        ...
        foreach ($data as $option) {
            if (is_array($value)) {
                if (in_array($option['value'], $value)) {
                    $html[] = '<option value="' . esc_attr($option['value']) . '" selected="selected">' . esc_attr($option['text']) . '</option>';
                } else {
                    $html[] = '<option value="' . esc_attr($option['value']) . '">' . esc_attr($option['text']) . '</option>';
                }
            } else {
                $html[] = '<option value="' . esc_attr($option['value']) . '" ' . ((esc_attr($option['value']) == $value) ? 'selected="selected"' : '') . '>' . esc_attr($option['text']) . '</option>';
            }
        }
        $html[] = '</select>';
    ...
    public static function vimeoGetVideo($video_id) {
        ...
        $result = array('success' => false, 'message' => '', 'data' => null);
        $videoObj = array();
        $urlFeed = 'https://vimeo.com/api/v2/video/' . $video_id . '.php';
        //Get noi dung vidoe tu video url feed
        $video = maybe_unserialize(wp_remote_retrieve_body(wp_remote_get($urlFeed)));
        if(!empty($video)){
            $video = $video[0];
        }
        ...
        if ($video) {//Xu ly neu video get thanh cong
            $videoObj['video_url'] = $video_id;
            $videoObj['large'] = $video['thumbnail_large'];
    ...
    public static function getImage($image_id, $source) {
        ...
        $rs_html = self::createHTML(json_encode($videoObj));
        $result['success'] = TRUE;
        $result['message'] = __('Image has been gotten', 'ap-background');
        $result['data'] = $rs_html;
        return $result;
    }
...
//        $pl_info = @simplexml_load_file("http://gdata.youtube.com/feeds/api/playlists/" . $playlistId);
        $pl_info = json_decode(wp_remote_retrieve_body(wp_remote_get('https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=' . $playlistId . '&key=' . $api)));
...
//        Lay ve thong tin danh sach cac video duong nguoi dung upload len
        $pl_info = json_decode(wp_remote_retrieve_body(wp_remote_get('https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=' . $username . '&key=' . $api)));
...
//            Duyet qua tung phan tu cua mang danh sahc video va gan video id vao $list_video
    ...
    public function vimeoGetVideoByUser($username) {
        $videos = maybe_unserialize(wp_remote_retrieve_body(wp_remote_get("https://vimeo.com/api/v2/$username/videos.php")));
        $list_video = array();
        if ($videos) {
            foreach ($videos as $video) {
                $list_video[] = $video['id'];
    ...
    public function vimeoGetVideoFromAlbum($albumId) {
        $dataURL = 'https://vimeo.com/api/v2/album/' . $albumId . '/videos.php';
        $videos = maybe_unserialize(wp_remote_retrieve_body(wp_remote_get($dataURL)));
        $list_video = array();
        if ($videos) {
            foreach ($videos as $video) {
                $list_video[] = $video['id'];
    ...
    static function createHTML($data) {
        ...
        $html[] = '</div>';
        $html[] = '</div>';
        $html[] = '<img ' . $attr . ' src="' . $datas->large . '"  alt="' . __('image preview', 'ap-background') . '"/>';
    ...
    public function flickrGetData($params) {
        ...
        $rsp = wp_remote_retrieve_body(wp_remote_get($url));
    ...
    public function getHostFromUrl($url) {
        $host = __('URL invalid or not support', 'ap-background');
    ...
    public function getPostIdsbyrateAvg($direction, $content_type = 'none') {
        ...
        $query .= ' WHERE p.post_type = "product" AND c.comment_approved=1  AND cm.meta_key="rating" group by p.ID ORDER BY avg_rate %s';
        $results = $wpdb->get_results($wpdb->prepare($query, $direction));
        if (!empty($results)) {
    ...
    public function is_product_in_wishlist($product_id) {
        ...
        $utinity = new btParallaxBackgroundUtility();
        $exists = false;

        if (is_user_logged_in() && defined('YITH_WCWL_TABLE')) {
            $sql = 'SELECT COUNT(*) as cnt FROM %s WHERE prod_id = %d AND user_id = %d';
            $results = $wpdb->get_results($wpdb->prepare($sql, YITH_WCWL_TABLE, $product_id, $this->details['user_id']));
            $exists = $results[0]->cnt > 0 ? true : false;
        } else {
            if (function_exists('yith_usecookies') && yith_usecookies()) {
                $tmp_products = yith_getcookie('yith_wcwl_products');
            } elseif (isset($_SESSION['yith_wcwl_products'])) {
                $tmp_products = $utinity->ap_clean_data($_SESSION['yith_wcwl_products']);
            } else {
                $tmp_products = array();
            }
    ...
    public function loadContentEffectCss($out, $in, $type) {
        ...
        if ($type == 'content') {
            $direct_path = SFAPB_PLUGIN_DIR . 'includes/css/parrallax-content/';
        }
        if ($type == 'item') {
            $direct_path = SFAPB_PLUGIN_DIR . 'includes/css/item/';
        }

        $default_content = $wp_filesystem->get_contents($direct_path . 'default.css');
        $i_content = $wp_filesystem->get_contents($direct_path . 'in.css');
        $o_content = $wp_filesystem->get_contents($direct_path . 'out.css');
        $in_content = $wp_filesystem->get_contents($direct_path . $in . '.css');
        $out_content = $wp_filesystem->get_contents($direct_path . $out . '.css');
        if ($default_content && $in_content && $out_content && $i_content && $o_content) {
            return wp_kses_post($default_content . $in_content . $i_content . $out_content . $o_content);
        } else {
            return false;
        }
    }
...
//        Kiem tra xem media item co ton tai hay khong
        if ($media_item) {//Neu ton tai thi lay kich thuoc cua file media item
            ...
            list($width, $height) = getimagesize(SFAPB_PLUGIN_DIR.'images/default/default_' . $index . '.png');
        }
        ...
        echo '<div class="parallax-row in-pos row-' . esc_attr($index) . '" style="' . esc_attr($item_style) . '">';
        if ($media_item) {
            echo '<div class="thumb" ><img ' . $attr . ' src="' . ($upload_dir['baseurl'] . '/ap_background/thumb/' . esc_attr($item->id) . '/' . esc_attr($media_item->thumbnail)) . '" alt="' . __('Image thumbnail','ap-background') . '"/></div>';
            echo '<div class="show_box hidden"><img class="image-show" src="' . (($media_item->media_source == 'image_upload') ? site_url() . esc_attr($media_item->large) : esc_url_raw($media_item->large)) . '" alt="' . __('Image large preview','ap-background') . '"/></div>';
        } else {
            echo '<div class="thumb" ><img ' . $attr . ' src="' . SFAPB_PLUGIN_URL . 'images/default/default_' . esc_attr($index) . '.png" alt="' . __('Image thumbnail', 'ap-background') . '"/></div>';
            echo '<div class="show_box hidden"><img class="image-show" src="' . SFAPB_PLUGIN_URL . 'images/default/default_large.png" alt="' . __('Image large preview', 'ap-background') . '"/></div>';
        }
        echo '</div>';
    }
    ...
    function checkMediaItem($id, $item) {
        $upload_dir = wp_upload_dir();
        $image_save = $upload_dir['basedir'] . '/ap_background/';
        ...
        if (!file_exists($upload_dir['basedir'] . '/ap_background/thumb/' . $id . '/' . $item->thumbnail)) {
            ...
            file_put_contents($upload_dir['basedir'] . '/ap_background/thumb/' . $id . '/' . $item->thumbnail, $file_content);
        }
    }

    function ap_clean_data($datas) {
        if (is_array($datas)) {
            foreach ($datas as $k => $v) {
                if (is_array($v)) {
                    return $this->ap_clean_data($v);
                } else {
                    if ($k == 'custom_effect_code' || $k == 'side_content') {
                        $datas[$k] = wp_kses_post($v);
                    } else {
                        $datas[$k] = sanitize_text_field($v);
                    }
                    return $datas;
                }
            }
        } else {
            return is_scalar($datas) ? sanitize_text_field($datas) : $datas;
        }
    }
